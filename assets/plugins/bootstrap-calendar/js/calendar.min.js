...
Date.prototype.getWeek = function () {
    var a = new Date(this.getUTCFullYear(), 0, 1);
    return Math.ceil(((this.getTime() - a.getTime()) / 864e5 + a.getUTCDay() + 1) / 7)
}, Date.prototype.getUTCMonthFormatted = function () {
    var a = this.getUTCMonth() + 1;
    return 10 > a ? "0" + a : a
    ...
    for (var b = "" + this; b.length < a;)b = "0" + b;
    return b
}), function (a) {
    function b(a, b) {
        var c, d, e;
        e = a, c = a.indexOf("?") < 0 ? "?" : "&";
        for (d in b)e += c + d + "=" + encodeURIComponent(b[d]), c = "&";
        return e
    }
    ...
    function d(b, f) {
        ...
        for (var j in i)g.push(j + ":" + i[j]);
        if (g.push(f), g = g.join("|"), g in d.cache)return d.cache[g];
        var k = [];
        return a.each(i, function (b, c) {
            var d = null, g = null, i = !1;
            if (a.each(b.split(">"), function (a, c) {
                    var j, k = null;
                    if (j = /^(\d\d)-(\d\d)$/.exec(c))k = new Date(f, parseInt(j[2], 10) - 1, parseInt(j[1], 10)); else if (j = /^(\d\d)-(\d\d)-(\d\d\d\d)$/.exec(c))parseInt(j[3], 10) == f && (k = new Date(f, parseInt(j[2], 10) - 1, parseInt(j[1], 10))); else if (j = /^easter(([+\-])(\d+))?$/.exec(c))k = h(f, j[1] ? parseInt(j[1], 10) : 0); else if (j = /^(\d\d)([+\-])([1-5])\*([0-6])$/.exec(c)) {
                        var l = parseInt(j[1], 10) - 1, m = j[2], n = parseInt(j[3]), o = parseInt(j[4]);
                        switch (m) {
                            case"+":
                                for (var p = new Date(f, l, -6); p.getUTCDay() != o;)p = new Date(p.getUTCFullYear(), p.getUTCMonth(), p.getUTCDate() + 1);
                                k = new Date(p.getUTCFullYear(), p.getUTCMonth(), p.getUTCDate() + 7 * n);
                                break;
                            case"-":
                                for (var p = new Date(f, l + 1, 7); p.getUTCDay() != o;)p = new Date(p.getUTCFullYear(), p.getUTCMonth(), p.getUTCDate() - 1);
                                k = new Date(p.getUTCFullYear(), p.getUTCMonth(), p.getUTCDate() - 7 * n)
                        }
                    }
                    if (!k)return e("Unknown holiday: " + b), i = !0, !1;
                    switch (a) {
                        case 0:
                            d = k;
                            break;
                        case 1:
                            if (k.getTime() <= d.getTime())return e("Unknown holiday: " + b), i = !0, !1;
                            g = k;
                            break;
                        default:
                ...
                var j = [];
                if (g)for (var l = new Date(d.getTime()); l.getTime() <= g.getTime(); l.setDate(l.getUTCDate() + 1))j.push(new Date(l.getTime())); else j.push(d);
    ...
    function f(b, c) {
        ...
        }, i, b), this.setLanguage(this.options.language), this.context = c, c.css("width", this.options.width).addClass("cal-context"), this.view(), this
    }
    ...
    function g(b, c, d, e) {
        ...
        c.fadeOut("fast"), d.slideUp("fast", function () {
            ...
            d.html(e.options.templates["events-list"]({
                cal: e,
                events: e.getEventsBetween(parseInt(b.data("cal-start")), parseInt(b.data("cal-end")))
            })), g.after(d), e.activecell = a("[data-cal-date]", f).text(), a("#cal-slide-tick").addClass("tick" + h).show(), d.slideDown("fast", function () {
    ...
    d.cache = {}, f.prototype.setOptions = function (b) {
        ...
        m.setHours(i[0]), m.setMinutes(i[1]), b.all_day = [], b.by_hour = [], b.after_time = [], b.before_time = [], a.each(b.events, function (a, d) {
            ...
            if (d.start_hour = f.getUTCHours().toString().formatNum(2) + ":" + f.getMinutes().toString().formatNum(2), d.end_hour = g.getUTCHours().toString().formatNum(2) + ":" + g.getMinutes().toString().formatNum(2), d.start < l.getTime() && (e(1), d.start_hour = f.getUTCDate() + " " + c.locale["ms" + f.getUTCMonth()] + " " + d.start_hour), d.end > m.getTime() && (e(1), d.end_hour = g.getUTCDate() + " " + c.locale["ms" + g.getUTCMonth()] + " " + d.end_hour), d.start < l.getTime() && d.end > m.getTime())return void b.all_day.push(d);
            if (d.end < l.getTime())return void b.before_time.push(d);
            if (d.start > m.getTime())return void b.after_time.push(d);
            var h = l.getTime() - d.start;
            d.top = h >= 0 ? 0 : Math.abs(h) / k + l.getTimezoneOffset()/15;
        ...
        return a.each(this.getEventsBetween(e, f), function (a, b) {
            b.start_day = new Date(parseInt(b.start)).getUTCDay(), 1 == i && (b.start_day = (b.start_day + 6) % 7), b.days = b.end - b.start <= 864e5 ? 1 : (b.end - b.start) / 864e5, b.start < e && (b.days = b.days - (e - b.start) / 864e5, b.start_day = 0), b.days = Math.ceil(b.days), b.start_day + b.days > 7 && (b.days = 7 - b.start_day), g.push(b)
        }), d.events = g, d.cal = this, h.options.templates["week-days"](d)
    }, f.prototype._month = function (a) {
        this._loadTemplate("year-month");
        var b = {cal: this}, c = a + 1;
        b.data_day = this.options.position.start.getUTCFullYear() + "-" + (10 > c ? "0" + c : c) + "-01", b.month_name = this.locale["m" + a];
        var d = new Date(this.options.position.start.getUTCFullYear(), a, 1, 0, 0, 0);
        return b.start = parseInt(d.getTime()), b.end = parseInt(new Date(this.options.position.start.getUTCFullYear(), a + 1, 1, 0, 0, 0).getTime()), b.events = this.getEventsBetween(b.start, b.end), this.options.templates["year-month"](b)
    }, f.prototype._day = function (b, d) {
        this._loadTemplate("month-day");
        var e = {
            tooltip: "",
            cal: this
        }, f = this.options.classes.months.outmonth, g = this.options.position.start.getUTCDay();
        2 == c(this, "first_day") ? g++ : g = 0 == g ? 7 : g, d = d - g + 1;
        var h = new Date(this.options.position.start.getUTCFullYear(), this.options.position.start.getUTCMonth(), d, 0, 0, 0);
        d > 0 && (f = this.options.classes.months.inmonth);
        var i = new Date(this.options.position.end.getTime() - 1).getUTCDate();
        if (d + 1 > i && (this.stop_cycling = !0), d > i && (d -= i, f = this.options.classes.months.outmonth), f = a.trim(f + " " + this._getUTCDayClass("months", h)), 0 >= d) {
            var j = new Date(this.options.position.start.getUTCFullYear(), this.options.position.start.getUTCMonth(), 0).getUTCDate();
            d = j - Math.abs(d), f += " cal-month-first-row"
        }
        var k = this._getHoliday(h);
        return k !== !1 && (e.tooltip = k), e.data_day = h.getUTCFullYear() + "-" + h.getUTCMonthFormatted() + "-" + (10 > d ? "0" + d : d), e.cls = f, e.day = d, e.start = parseInt(h.getTime()), e.end = parseInt(e.start + 864e5), e.events = this.getEventsBetween(e.start, e.end), this.options.templates["month-day"](e)
    }, f.prototype._getHoliday = function (b) {
        var c = !1;
        return a.each(d(this, b.getUTCFullYear()), function () {
            var d = !1;
        ...
        return b === !1 ? "" : b
    }, f.prototype._getUTCDayClass = function (a, b) {
        ...
        switch (f !== !1 && d("holidays", e), b.getUTCDay()) {
            case 0:
                d("sunday", e);
                break;
            case 6:
        ...
        if (a) {
            if (!this.options.views[a].enable)return;
            this.options.view = a
        }
        ...
        if ("next" == b)switch (this.options.view) {
            case"year":
                d.start.setFullYear(this.options.position.start.getUTCFullYear() + 1);
                break;
            case"month":
                d.start.setMonth(this.options.position.start.getUTCMonth() + 1);
                break;
            case"week":
                d.start.setDate(this.options.position.start.getUTCDate() + 7);
                break;
            case"day":
                d.start.setDate(this.options.position.start.getUTCDate() + 1)
        } else if ("prev" == b)switch (this.options.view) {
            case"year":
                d.start.setFullYear(this.options.position.start.getUTCFullYear() - 1);
                break;
            case"month":
            ...
            case"day":
                d.start.setDate(this.options.position.start.getUTCDate() - 1)
        } else"today" == b ? d.start.setTime((new Date).getTime()) : a.error(this.locale.error_where.format(b));
        this.options.day = d.start.getUTCFullYear() + "-" + d.start.getUTCMonthFormatted() + "-" + d.start.getUTCDateFormatted(), this.view(), _.isFunction(c) && c()
    }, f.prototype._init_position = function () {
        var b, d, e;
        if ("now" == this.options.day) {
            var f = new Date;
            b = f.getUTCFullYear(), d = f.getUTCMonth(), e = f.getUTCDate()
        } else if (this.options.day.match(/^\d{4}-\d{2}-\d{2}$/g)) {
            var g = this.options.day.split("-");
            b = parseInt(g[0], 10), d = parseInt(g[1], 10) - 1, e = parseInt(g[2], 10)
        } else a.error(this.locale.error_dateformat.format(this.options.day));
        switch (this.options.view) {
            ...
            case"month":
                ...
                break;
            case"day":
                var start_day = new Date(b, d, e), end_day = new Date(b, d, e + 1);
                this.options.position.start.setTime(start_day.getTime() - start_day.getTimezoneOffset()*60*1000), this.options.position.end.setTime(end_day.getTime() - end_day.getTimezoneOffset()*60*1000);
                break;
            case"week":
                var h, i = new Date(b, d, e);
                h = 1 == c(this, "first_day") ? i.getUTCDate() - (i.getUTCDay() + 6) % 7 : i.getUTCDate() - i.getUTCDay(), this.options.position.start.setTime(new Date(b, d, h).getTime()), this.options.position.end.setTime(new Date(b, d, h + 7).getTime());
                break;
            default:
        ...
        switch (this.options.view) {
            case"year":
                return this.locale.title_year.format(a.getUTCFullYear());
            case"month":
                return this.locale.title_month.format(this.locale["m" + a.getUTCMonth()], a.getUTCFullYear());
            case"week":
                return this.locale.title_week.format(a.getWeek(), a.getUTCFullYear());
            case"day":
        ...
        switch (a.type(d)) {
            ...
            case"string":
                d.length && (f = function () {
                    var e = [], f = {
                        from: c.options.position.start.getTime(),
                        to: c.options.position.end.getTime()
                    };
                    return l.length && (f.browser_timezone = l), a.ajax({
                        url: b(d, f),
                        dataType: "json",
                        type: "GET",
                        async: !1
        ...
        if (a("a[data-event-id]", this.context).unbind("click"), b.options.modal) {
            ...
            if (c.length) {
                var d = null;
                "iframe" == b.options.modal_type && (d = a(document.createElement("iframe")).attr({
                    width: "100%",
                    frameborder: "0"
                })), a("a[data-event-id]", this.context).on("click", function (e) {
                    ...
                    "iframe" == b.options.modal_type && (d.attr("src", f), a(".modal-body", c).html(d)), (!c.data("handled.bootstrap-calendar") || c.data("handled.bootstrap-calendar") && c.data("handled.event-id") != e.id) && c.off("show.bs.modal").off("shown.bs.modal").off("hidden.bs.modal").on("show.bs.modal", function () {
                        ...
                        switch (b.options.modal_type) {
                            ...
                            case"ajax":
                                ...
                                break;
                            case"template":
                                b._loadTemplate("modal"), d.html(b.options.templates.modal({
                                    event: e,
                                    calendar: b
                                }))
                        }
        ...
        this._update_month_year();
        var b = this, c = a(document.createElement("div")).attr("id", "cal-week-box"), d = this.options.position.start.getUTCFullYear() + "-" + this.options.position.start.getUTCMonthFormatted() + "-";
